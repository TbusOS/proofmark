...
`hardware-3d` 是这里的另类。那九套是排一张页面，这一套是渲染一张页面 —— 单个 HTML、零外部依赖、手写的 WebGL2 引擎，用来讲一块硬件到底是怎么工作的。

照片感不是靠某个 shader 技巧。相机一停，每帧重新抖动四样东西 —— 亚像素偏移（高斯，σ≈0.32px）、光圈盘、柔光箱面积、环境光遮蔽的采样核 —— 然后平均。两百次采样之后，软阴影和景深是采出来的，不是近似出来的。移轴把焦平面放平贴着板面，而不是垂直于视线，整块板从这个角到那个角都清楚。

更难的是数据流那一半，也正是这套东西存在的理由。一个 64 位字画成一行亮暗格子，不是一个点。一条总线是 64 根并行导线，不是一条线。路径走在 **die 内部** —— 逻辑块躺在硅上，架空的铜轨在它们上方，那正是金属互连层的物理样子。全部按时钟拍推进，配暂停、单步和速度滑块，延迟让读者自己数，而不是写一句话告诉他。

它自带四道能跑的检查，跟设计检查链是一个路数：

| 检查 | 什么情况不过 |
|---|---|
| `check_selfcontained.sh` | 页面引了 Google Fonts 以外的任何外部资源 |
| `check_pipeline.mjs` | 缺渲染通道、有 GL 错误、或者某一站收敛不了 |
| `check_realism.mjs` | 画面又飘回「深蓝加霓虹」 |
| `check_perf.mjs` | 单次累积帧超时 |

第三道对应设计检查链里的品牌检查 —— 把风格量出来，而不是描述出来。正常页面高亮高饱和像素占 0.4%、饱和度中位数 0.09；把材质染成霓虹青之后是 44.8% 和 0.63，检查当场拦下。

**[做完的页面：一条数据从内存颗粒走到 CPU 寄存器](demos/hardware-3d/index.html)** —— 五站：棚拍下的整板、一颗 DRAM 封装的微距、颗粒内部的行列寻址、板上的 64 根数据线、以及穿过 L3 / L2 / L1 排队进寄存器的那一路。

## 做完的页面

用那九套做完、并且过了检查链的 26 张页面，在 `demos/` 下。
...
十三个 skill、六个评审 agent、两个命令，放到 Claude Code 找得到的地方。
